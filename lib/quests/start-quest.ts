...
function mapPublicQuest(quest: QuestRecord): PublicQuest {
  return {
    id: quest.id,
...
function mapPublicScene(scene: QuestSceneRecord): PublicQuestScene {
  return {
    id: scene.id,
    actId: scene.act_id,
...
function getScenePoints(scene: QuestSceneRecord): number {
...
function calculateMaxScore(scenes: QuestSceneRecord[]): number {
  return scenes.reduce((total, scene) => total + getScenePoints(scene), 0);
}
...
function buildProgress(
  ...
  return {
    current: Math.min(run.completed_scene_count + 1, totalScenes),
    total: totalScenes,
...
async function loadQuestStructure(questId: string): Promise<QuestStructure> {
  const admin = createAdminClient();
...
async function removeFailedRun(runId: string): Promise<void> {
  ...
  const { error } = await admin.from("quest_runs").delete().eq("id", runId);

  if (error) {
    console.error("Failed to clean up quest run:", error);
  }
}
...
export async function startQuest({
  ...
  const normalizedCampaignSlug = normalizeSlug(campaignSlug);

  const normalizedEpisodeSlug = normalizeSlug(episodeSlug);

  const normalizedQuestSlug = normalizeSlug(questSlug);
  ...
  if (!normalizedEpisodeSlug) {
    throw new QuestEngineError("EPISODE_NOT_FOUND", "Episode slug is required");
  }

  if (!normalizedQuestSlug) {
    throw new QuestEngineError("QUEST_NOT_FOUND", "Quest slug is required");
  }

  const campaign = await loadCampaign(normalizedCampaignSlug);

  const episode = await loadEpisode(campaign.id, normalizedEpisodeSlug);

  const quest = await loadQuest(episode.id, normalizedQuestSlug);

  const { acts, scenes } = await loadQuestStructure(quest.id);

  const activeRun = await loadActiveRun(userId, quest.id);
  ...
  const firstAct = acts.find((act) => act.id === firstScene.act_id);
  ...
  const runInsert: QuestRunInsert = {
    ...
    current_scene_code: firstScene.scene_code,
    completed_scene_count: 0,
    score: 0,
    max_score: calculateMaxScore(scenes),
    xp_earned: 0,
    coins_earned: 0,
    state: {
      ...
      currentActCode: firstAct.act_code,
  ...
  if (runError || !runData) {
    ...
    if (runError?.code === "23505") {
      const concurrentRun = await loadActiveRun(userId, quest.id);
    ...
    console.error("Failed to create quest run:", runError);
  ...
  try {
    await recordQuestEvent({
      ...
      metadata: {
        resumed: false,
        actId: firstScene.act_id,
        orderIndex: firstScene.order_index,
      },
    });
  } catch (error) {
