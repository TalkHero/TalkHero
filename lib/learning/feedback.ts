...
function parseCorrectionBlock(
  block: string,
): {
  originalFragment?: string;
  correctedFragment?: string;
} | null {
  const content = stripPrefix(
    block,
    CORRECTION_PREFIXES,
  );
...
export function normalizeFeedback(
  ...
  if (typeof feedback !== "string") {
    return {
      ...
      remember:
        feedback.remember?.trim() ||
        undefined,
      npcReply:
        feedback.npcReply?.trim() ||
        undefined,
              errorType:
        feedback.errorType,

      originalFragment:
        feedback.originalFragment
          ?.trim() ||
        undefined,

      correctedFragment:
        feedback.correctedFragment
          ?.trim() ||
        undefined,
      category:
        feedback.category,
