...
export function useQuest() {
  ...
  const [evaluation, setEvaluation] = useState<QuestSceneEvaluation | null>(null);
  ...
  const startQuest = useCallback(async (params: StartQuestParams) => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Failed to start quest");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const submitAnswer = useCallback(async ({
    ...
    try {
      const response = await fetch("/api/quests/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ runId, userInput, responseTimeMs }),
      });
